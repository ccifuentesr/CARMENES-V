...
# PLOT: ABSOLUTE MAGNITUDE - COLOUR
            ...
            if row['d_pc'] != '':
                ...
                if row['Qf_'+y_axis] == row['Qf_'+x_axis[0]] == row['Qf_'+x_axis[1]] == 'false':
                    if row[booleans[1]] == 'true':
                        SpT_y.append(float(row['SpTnum']))
                        d_pc_y.append(float(row['d_pc']))
                        mag0_y.append(float(row[x_axis[0]+'_mag']))
                        mag1_y.append(float(row[x_axis[1]+'_mag']))
                        mag2_y.append(float(row[y_axis+'_mag']))
...
colour_x_y = [mag0_y[i] - mag1_y[i] for i in range(len(mag0_y))]
absmag_y = [mag2_y[i] - 5 *
            np.log10(d_pc_y[i]) + 5 for i in range(len(mag2_y))]
...
ax.scatter(x_cif_y, y_cif_y, marker='o', s=pointsize,
